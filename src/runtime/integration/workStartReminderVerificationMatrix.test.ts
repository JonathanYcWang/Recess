...
describe('work start reminder verification matrix', () => {
  describe('schedule management', () => {
    ...
    it(
      'marks skip next neutral without changing streak or coins',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
  ...
  describe('outcomes, streak, and coins', () => {
    it(
      'satisfies within the inclusive window and advances streak with ten Coins',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
        await harness.reminder.addSchedule({ time: '09:00 AM', days: [...ALL_DAYS] });
    ...
    it(
      'resets streak on missed deadlines without awarding Coins',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
        await harness.reminder.addSchedule({ time: '09:00 AM', days: [...ALL_DAYS] });
        const before = await harness.readState();
    ...
    it(
      'coalesces overlapping schedules into one streak and coin consequence',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
        await harness.reminder.addSchedule({ time: '09:00 AM', days: [...ALL_DAYS] });
  ...
  describe('recovery and notification contracts', () => {
    it(
      'preserves deterministic outcomes after restart',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
    ...
    it(
      'keeps occurrence truth when notification delivery fails',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness({
        ...
        const reconciled = await reconcileReminderAlarmAt(harness, planned);
    ...
    it(
      'reconciles duplicate alarms idempotently',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
        ...
        const first = await reconcileReminderAlarmAt(harness, planned);
    ...
    it(
      'migrates legacy workHours storage once and removes the legacy key',
      async () => {
  ...
  describe('boundary and timezone coverage', () => {
    it(
      'treats scheduled and window-close instants as inclusive',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
  ...
  describe('work session fact integration', () => {
    it(
      'resolves reminder outcomes when a work session starts through work rhythm',
      async () => {
        const harness = await createWorkStartReminderVerificationHarness();
