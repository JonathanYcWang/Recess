...
import { createBackgroundCompositionRoot } from './backgroundCompositionRoot';
...
export const registerSettingsRuntimeListener = (options: {
  ...
  rootReady = createBackgroundCompositionRoot({ adapter: options.adapter }).then((root) => {
    if (!root.ok) {
      return;
    }
    registration = { handler: root.value.handler };
  });
