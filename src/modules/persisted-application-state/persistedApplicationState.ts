import { documentRegistry, registeredDocumentNames, type DocumentRegistryEntry, } from './registry/documentRegistry';
import type { CommitError, CommitResult, HydrationSnapshot, KeyValueStorageAdapter, PersistedApplicationState, PersistedChangeListener, PersistedDocumentName, PersistedDocuments, Result, StorageError, VersionedDocument, } from './types';
...
const runSerialized = <T>(task: () => Promise<T>): Promise<T> => {
    ...
    commitChain = next.then(() => undefined, () => undefined);
    return next;
};
const readDocument = async <T>(adapter: KeyValueStorageAdapter, entry: DocumentRegistryEntry<T>): Promise<Result<VersionedDocument<T>, StorageError | import('./types').CodecError>> => {
...
const writeDocument = async <T>(adapter: KeyValueStorageAdapter, entry: DocumentRegistryEntry<T>, document: VersionedDocument<T>, expectedRevision: number): Promise<Result<VersionedDocument<T>, CommitError>> => {
...
export const createPersistedApplicationState = (options: PersistedApplicationStateOptions): PersistedApplicationState => {
    ...
    const loadDocumentWithRecovery = async <K extends PersistedDocumentName>(name: K): Promise<VersionedDocument<PersistedDocuments[K]>> => {
    ...
    return {
        async initialize(): Promise<Result<HydrationSnapshot, StorageError>> {
            ...
            for (const name of registeredDocumentNames) {
                (documents as Record<PersistedDocumentName, VersionedDocument<PersistedDocuments[PersistedDocumentName]>>)[name] = await loadDocumentWithRecovery(name);
            }
        ...
        commit(mutations) {
            return runSerialized(async () => {
                ...
                for (const mutation of mutations) {
                    const entry = documentRegistry[mutation.document] as DocumentRegistryEntry<PersistedDocuments[typeof mutation.document]>;
                    const current = await readDocument(adapter, entry);
                    if (!current.ok) {
                        return { ok: false, error: { kind: 'codec', error: current.error } } as Result<CommitResult, CommitError>;
                    }
                    const nextValue = {
                        ...current.value,
                        value: mutation.value,
                    };
                    const committed = await writeDocument(adapter, entry, nextValue, mutation.expectedRevision);
                    if (!committed.ok) {
                        return committed as Result<CommitResult, CommitError>;
                    }
                    (result as Record<PersistedDocumentName, VersionedDocument<PersistedDocuments[PersistedDocumentName]> | undefined>)[mutation.document] = committed.value;
                }
                notify(result);
